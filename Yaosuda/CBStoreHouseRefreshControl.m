...
{
    return MIN(1.f, MAX(0,fabsf((float)self.realContentOffsetY)/self.dropHeight));
}
...
- (void)finishingLoading
{
    ...
    [self.displayLink addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];//最优先级

    
    //    mytimer=[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(updateDisappearAnimation) userInfo:nil repeats:YES];
//    
